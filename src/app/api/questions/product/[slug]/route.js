...
export async function GET(req, { params }) {
  try {
    ...
    if (!exam) {
      return new Response(JSON.stringify({ success: false, message: 'Exam not found for this product' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    console.error('❌ Error fetching questions by product slug:', error.message);
