...
export async function POST(request) {
  try {
    ...
    if (order.result.status !== 'APPROVED' && order.result.status !== 'COMPLETED') {
      console.error('Invalid order status:', { orderId, status: order.result.status });
      return NextResponse.json({ success: false, error: 'Invalid order status' }, { status: 400 });
    }

    // Verify amount
    const paypalAmount = parseFloat(order.result.purchase_units[0].amount.value);
    if (paypalAmount !== parseFloat(amount)) {
      console.error('Amount mismatch:', { provided: amount, actual: paypalAmount });
